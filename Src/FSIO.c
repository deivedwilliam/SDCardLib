...
// Directory functions
    ...
    unsigned char str_put_n_char (FSFILE* handle, unsigned char n, char c);
    ...
    while(1)
    {
        switch (i)
        {
            ...
            case '.':
                ...
                if(i == '.')
                {
                	temppath++;				// Increment the path variable
                    i = *temppath;

                    if(tempCWD->dirclus == FatRootDirClusterValue)	// Check if we're in the root
                    {
            ...
            case '\\':
				...
				temppath++;	 		// Increment pointer to second char
				i = *temppath;
